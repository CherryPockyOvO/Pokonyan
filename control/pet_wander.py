...
class PetWanderController:
    ...
    2. Pause duration is kept short (0.4 ~ 1.0s) so it moves around like a pet.
    3. Excludes backward commands (S, SA, SD).
    4. Automatically avoids obstacles using ultrasonic telemetry (turns left or right until clear).
    """
    ...
    def __init__(
        ...
        self.avoid_direction = "A"  # 'A' (left) or 'D' (right)
        ...
        self.actions = {
            "W": {
                "pwm": (self.forward_pwm, self.forward_pwm),
                "duration": (1.5, 3.5),
                "weight": 40,
                "name": "Forward (W)",
            },
            "WA": {
                "pwm": (self.inner_pwm, self.forward_pwm),
                "duration": (1.0, 2.5),
                "weight": 20,
                "name": "Forward-Left (WA)",
            },
            "WD": {
                "pwm": (self.forward_pwm, self.inner_pwm),
                "duration": (1.0, 2.5),
                "weight": 20,
                "name": "Forward-Right (WD)",
            },
            "A": {
                "pwm": (-self.pivot_pwm, self.pivot_pwm),
                "duration": (0.5, 1.2),
                "weight": 8,
                "name": "Turn-Left (A)",
            },
        ...
        with self.lock:
            ...
            if is_obstacle or self.mode == "AVOID_OBSTACLE":
                ...
                if path_cleared:
                    ...
                    return self.command, self.reason
