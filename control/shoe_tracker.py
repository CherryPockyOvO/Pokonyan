...
class ShoeTrackerController:
    ...
    def tick(self, target, distance_cm):
        ...
        with self.lock:
            ...
            if shoe_size_ratio >= 0.16:
                ...
                if self.pulse_state == "PULSING":
                    if now < self.pulse_until:
                        ...
                        return (0, 0), "🎯 Near Mode (>=16%): Step pulse pause 1.0s (inspecting frame)"
            ...
            # -------------------------------------------------------------
            # 階段 3: 鞋子面積/高度小於 16% (< 0.16) -> 遠距離模式 (實時消抖更新，組合鍵 WA/WD 逼近)
            # -------------------------------------------------------------
            self.pulse_state = "IDLE"
            if abs(dx) <= self.deadband_px:
                cmd = (220, 220)
                self.reason = f"🎯 Far Pursuit (Ratio {shoe_size_ratio*100:.1f}% < 16%): Centered -> Drive Forward (220, 220)"
            elif dx < 0:
                cmd = (80, 220)  # 左前弧線 WA (80, 220)
                self.reason = f"🎯 Far Pursuit (Ratio {shoe_size_ratio*100:.1f}% < 16%): Curved Left (WA: 80, 220)"
            else:
                cmd = (220, 80)  # 右前弧線 WD (220, 80)
                self.reason = f"🎯 Far Pursuit (Ratio {shoe_size_ratio*100:.1f}% < 16%): Curved Right (WD: 220, 80)"
