    ...
    def do_POST(self):
        if self.path == "/set_shoe_tracking":
            ...
            try:
                ...
                global shoe_tracking_enabled
                shoe_tracking_enabled = enabled
                print(f"\n👟 [Port 5000 Bridge] User changed Shoe Tracking switch -> {enabled}")

                def forward_to_pi():
                    try:
                        url = f"http://{PI_HOST}:{PI_PORT}/set_shoe_tracking"
                        ...
                        pass
                threading.Thread(target=forward_to_pi, daemon=True).start()
            except Exception:
                pass

            self.send_json({"ok": True, "enabled": shoe_tracking_enabled})
            return
